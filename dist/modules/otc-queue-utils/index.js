define("@modules/otc-queue-utils", ["require", "exports", "@modules/global", "@ijstech/eth-wallet", "@modules/store", "@scom/oswap-openswap-contract", "@scom/commission-proxy", "@scom/oswap-chainlink-contract", "@ijstech/components"], function (require, exports, global_1, eth_wallet_1, store_1, oswap_openswap_contract_1, commission_proxy_1, oswap_chainlink_contract_1, components_1) {
    "use strict";
    ...
    const hybridTradeExactIn = async (wallet, path, pairs, amountIn, amountOutMin, toAddress, deadline, data, commissionFee, commissionFeeTo, callback, confirmationCallback) => {
        ...
        const proxy = new commission_proxy_1.Contracts.Proxy(wallet, proxyAddress);
    ...
    const getOffers = async (params) => {
        ...
        const restrictedPrice = new eth_wallet_1.BigNumber(offer.restrictedPrice).times(tradeFee).shiftedBy(-18).toFixed();
        const offerPrice = global_1.toWeiInv(restrictedPrice).shiftedBy(-18).toFixed();
        return Object.assign(Object.assign({ pairAddress, totalAmount: new eth_wallet_1.BigNumber(originalAmount).times(offerPrice).dividedBy(tradeFee), availableAmount: new eth_wallet_1.BigNumber(amount).times(offerPrice).shiftedBy(-Number(tokenIn.decimals)), tradeFee,
